...
// Like configuring an email client
...
async function checkAndSendAlerts() {
  ...
  try {
    ...
    // Group by user so one user gets one email
    const byUser = {};
    allAlerts.forEach(med => {
      const email = med.user.email;
      if (!byUser[email]) {
        byUser[email] = { expiring: [], expired: [] };
      }
    ...
    for (const [userEmail, data] of Object.entries(byUser)) {
      const expiringHTML = data.expiring.map(m =>
        `<li><strong>${m.name}</strong> ${m.dosage || ''}
         — expires in <strong>${m.daysLeft} days</strong></li>`
      ).join('');

      const expiredHTML = data.expired.map(m =>
        `<li><strong>${m.name}</strong> ${m.dosage || ''}
         — <span style="color:red">EXPIRED</span></li>`
      ).join('');
    ...
    <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; background: #070b12; color: #eef2ff; border-radius: 16px; overflow: hidden;">
      ...
      <div style="padding: 28px 32px;">
        ...
        ${data.expired.length > 0 ? `
          <div style="background: rgba(245,101,101,0.1); border: 1px solid rgba(245,101,101,0.3); border-radius: 12px; padding: 18px; margin-bottom: 20px;">
        ...
        ${data.expiring.length > 0 ? `
          <div style="background: rgba(245,158,11,0.08); border: 1px solid rgba(245,158,11,0.25); border-radius: 12px; padding: 18px; margin-bottom: 20px;">
